...
[![Postman Documenter](https://img.shields.io/badge/Postman-Live%20Docs-FF6C37?style=for-the-badge&logo=postman&logoColor=white)](https://documenter.getpostman.com/view/56912926/2sBYAsyY86)
...
16. [Postman Collections & Live Documentation](#-postman-collections--live-api-documentation)
...
| **Live Docs** | **Postman Documenter** | `v2.1` | Published public interactive documentation and API testing runner. |
...
---
## 📬 Postman Collections & Live API Documentation
### 🌐 Official Published Postman Documentation:
The complete, interactive API documentation is published and hosted on Postman Documenter:
👉 **[View Live Postman API Documentation](https://documenter.getpostman.com/view/56912926/2sBYAsyY86)**
---
### 📦 Local Collection Files (Included in Root):
- 📄 `Recruitment_Platform_API_Final_Edition.postman_collection.json` — **(Recommended)** Complete end-to-end automated suite with automated JWT token extraction, OTP generation/verification tests, interview scoring thresholds, dynamic variable propagation, and edge-case/negative test suites.
- 📄 `Recruitment_Platform_API.postman_collection.json` — Standard baseline collection covering all CRUD operations across the 6 microservices.
### 🛠️ How to Import & Run in Postman:
1. Open **Postman** $\rightarrow$ Click **Import** (top-left).
2. Drag and drop `Recruitment_Platform_API_Final_Edition.postman_collection.json`.
3. Select the collection $\rightarrow$ Click **Run Collection** (Runner tab).
4. Run all folders in sequential order. The automated test scripts will:
   - Auto-register and login the HR user.
   - Automatically extract and inject the `Bearer {{token}}` into subsequent calls.
   - Create a Job, register a Candidate, submit an Application, track the `UUID` status, schedule an Interview, record an Evaluation score, and Hire the candidate.
   - Run 12+ negative and security validation tests.
---
